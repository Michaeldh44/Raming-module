...
function leegProject(){return{nr:'',adres:'',datum:new Date().toLocaleDateString('nl-NL'),calctoeslag:5,
  ...
  horizon:50,rente:2.0,daken:[nieuwDak('A')]};}
...
function tcoCalc(inv, levensduur){
  const H=parseFloat(project.horizon)||50;const r=(parseFloat(project.rente)||2)/100;
  const ohJaar=(TCO.onderhoud_vast_jaar||1000);
  const nulBedrag=(TCO.nulmeting_bedrag||2000);const nulInt=(TCO.nulmeting_interval||5);
  const overlaagFac=(TCO.overlaag_factor||0.55);
  let npv=inv,nominaal=inv,overlagingen=0;const at=(b,j)=>b/Math.pow(1+r,j);
  for(let j=1;j<=H;j++){
    // vast jaarlijks onderhoud (complex)
    npv+=at(ohJaar,j);nominaal+=ohJaar;
    // nulmeting elke nulInt jaar
    if(j%nulInt===0){npv+=at(nulBedrag,j);nominaal+=nulBedrag;}
    // einde levensduur -> overlaging (nieuwe toplaag), niet volledige vervanging
    if(levensduur>0 && j%Math.round(levensduur)===0 && j<H){const ov=inv*overlaagFac;npv+=at(ov,j);nominaal+=ov;overlagingen++;}
  }
  return{npv,nominaal,levensduur:Math.round(levensduur),ohJaar,overlagingen,perJaar:npv/H};
}
...
function tcoVoorDaktype(dk,inv){return tcoCalc(inv,levensduurVoorDaktype(dk));}
...
function tcoVoorMethode(mk,inv){const dk=dominantDaktype();const mm=TCO.methodes[mk];
  const ld=levensduurVoorDaktype(dk)*(mm.levensduur_factor||1);
  return tcoCalc(inv,ld);}
// TCO per bitumen-product (Wedeflex/Derbigum/Soprema) met eigen levensduur
function tcoVoorProduct(prodKey,inv){return tcoCalc(inv,PROD[prodKey].levensduur);}
...
function tcoTabel(titel, rows, kolTitel){
  const maxNpv=Math.max(...rows.map(r=>r.npv));const best=rows.reduce((a,b)=>b.npv<a.npv?b:a,rows[0]);const H=parseFloat(project.horizon)||50;
  return `<div class="tcosub">${titel}</div>
    <table class="tcotable"><thead><tr><th class="l">${kolTitel}</th><th>Levens&shy;duur</th><th>Investering</th><th>Overla&shy;gingen</th><th class="npv">TCO over ${H} jr (NPV)</th><th>Per jaar</th></tr></thead><tbody>
    ${rows.map(r=>`<tr class="${r===best?'best':''}">
      <td class="l">${r===best?'<span class=\'mark\'>✓</span> ':''}${r.naam}</td>
      <td>${r.levensduur} jr</td><td>${fmt(r.inv)}</td><td>${r.overlagingen}×</td>
...
function tcoBlok(T){
  const H=parseFloat(project.horizon)||50;const bpk=bouwplaatskosten().totaal;
  // Tabel 1: werkmethodes (met dominant daktype/product als basis)
  ...
  // Tabel 2: de 3 bitumen-producten (elk met eigen levensduur), investering = project met dat product op bitumen-gebrand daken
  const prodRows=Object.keys(PROD).map(pk=>{
    let inv=0;project.daken.forEach(d=>{
      // dwing product af op bitumen_gebrand daken; overige daktypes tellen normaal mee
      const savedProd=d.product; d.product=pk;
      inv+=berekenDak(d,d.daktype,gekozenMethode(d)).vk;
      d.product=savedProd;
    });inv+=bpk;
    const t=tcoVoorProduct(pk,inv);
    return{pk,naam:PROD[pk].naam,inv,...t};
  });
  const domNaam=(dominantProduct()?PROD[dominantProduct()].naam:DT[dominantDaktype()].naam.split(' (')[0]);
  const oh=(TCO.onderhoud_vast_jaar||1000);const nb=(TCO.nulmeting_bedrag||2000);const ni=(TCO.nulmeting_interval||5);
  return `<div class="tco"><h2>TCO-vergelijking over ${H} jaar (contant, rente ${project.rente}%)</h2>
    ...
    <div class="hint" style="margin-bottom:16px">Onderhoud complexbreed: € ${oh.toLocaleString('nl-NL')}/jaar + nulmeting € ${nb.toLocaleString('nl-NL')} elke ${ni} jaar. Bij einde levensduur wordt <b>overlaagd</b> (nieuwe toplaag), geen volledige vervanging.</div>
    ${tcoTabel('1 · Werkmethode vergeleken <span class="tcodim">(basis: '+domNaam+')</span>',methodeRows,'Werkmethode')}
    <div class="hint" style="margin:8px 0 18px">Overlagen bouwt op de bestaande ondergrond en gaat daardoor korter mee. Bij-isoleren en slopen &amp; opbouwen geven de volle levensduur; het verschil zit in de kosten vooraf (sloop + afvoer), niet in de levensduur.</div>
    ${tcoTabel('2 · Toplaag/product vergeleken <span class="tcodim">(op bitumen-gebrand daken)</span>',prodRows,'Product')}
    <div class="hint" style="margin-top:10px">TCO = investering + contant gemaakte onderhoudskosten, nulmetingen en overlagingen over ${H} jaar. De rij met ✓ heeft de laagste TCO. Soprema is goedkoper vooraf maar moet door de kortere levensduur vaker overlaagd worden; Wédéflex en Derbigum gaan langer mee.</div>
  </div>`;
}
...
function exportXls(){
  ...
  tr('');tr('<b>2 · Toplaag/product</b>','investering','TCO','levensduur');
  Object.keys(PROD).forEach(pk=>{let inv=0;project.daken.forEach(d=>{const sp=d.product;d.product=pk;inv+=berekenDak(d,d.daktype,gekozenMethode(d)).vk;d.product=sp;});inv+=bpk.totaal;const t=tcoVoorProduct(pk,inv);tr(PROD[pk].naam,fmt(inv),fmt(t.npv),t.levensduur+' jr');});
